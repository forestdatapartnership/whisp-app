...
import { Moon, Sun, User, Settings, LayoutDashboard, BriefcaseBusiness, LogOut } from "lucide-react";
...
const ACCOUNT_LINKS = [
  ...
  { icon: BriefcaseBusiness,  href: "/dashboard/jobs", label: "Analysis Jobs" },
];
...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 flex h-14 items-center gap-4 border-b border-border bg-bg px-8">
      <Link href="/" className="flex items-center gap-2.5 no-underline">
        <Image src="/whisp_logo_2_white.svg" alt="Whisp" width={60} height={60} />
      </Link>
      ...
      <Tooltip>
        <TooltipTrigger render={<Button variant="ghost" size="icon" />}>
          <a href="https://github.com/openforis/whisp" target="_blank" rel="noopener noreferrer">
      ...
      {isAuthenticated ? (
        <div className="relative" ref={menuRef} onBlur={handleOutsideClick}>
          ...
          {open && (
            <div
              role="menu"
              className="absolute right-0 top-[calc(100%+8px)] z-50 min-w-44 rounded-lg border border-border bg-surface py-1 shadow-md"
            >
              {user?.email && (
        ...
        <div className="flex shrink-0 overflow-hidden rounded-lg border border-border">
          <Button variant="ghost" size="sm" className="rounded-none" nativeButton={false} render={<Link href="/login" />}>
            Sign in
          </Button>
          <Button variant="ghost" size="sm" className="rounded-none border-l border-border hover:text-accent-green" nativeButton={false} render={<Link href="/register" />}>
            Register
          </Button>
        </div>
      )}
